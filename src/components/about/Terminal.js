...
function Terminal(props) {
   const {text, isBlog} = props;

   return (
      <Box component={'section'} className={classNames(Style.terminal, Style.shadowed)}
           width={{xs: '90vw' , md: `${isBlog?90:50}%`}} borderRadius={'0.5rem'} mb={'4rem'}>
         ...
         <Box py={{xs: '1rem', md: '2rem'}} px={{xs: '2rem', md: '3rem'}} borderRadius={'0 0 0.5rem 0.5rem'}
              sx={{backgroundColor: '#27242f'}} fontSize={{xs:`${isBlog?1:1.25}rem`, md:`${isBlog?1.25:1.5}rem`}} fontFamily={'monospace, Courier New, Courier'}>
            {text}
         </Box>
      </Box>
   );
